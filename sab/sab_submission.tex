...
\begin{abstract}
  An agent acting in an enviromnet aims to minimise uncertainties so
  that being attacked can be predicted and that rewards are not just
  found by chance. This defines an error signal which can be used to
  become better. In this paper we present a new algorithm where an
  error signal from a reflex trains a novel network where the error is
  propagated from its input to its output to generate pro-active
...
\begin{figure}[h!]
  ...
  \caption{A) Network overview. Except for the input layer, every
    neuron is a composite cell with an activation $v$ and and an error
...
(e.g. the ‘flashes’ that happen when an ammo box respawns). The reflex
...
image (Fig.~\ref{shooter_results}G), so it is forced to discover
...
threshold of the image centre, the bot fires. Instead of separate
...
\begin{figure}[h!]
	...
	\caption{A) KD at end of trial. B) Time to success vs learning
          rate. C) Typical output in time. D) KD learning curves. E)
          Typical weight growth. F) Example input weights. G) Example
          input frame.
...
of the error signal (Fig.~\ref{shooter_results}E).
...
do statistics, we measure the time taken to reach a threshold KD -
...
generate a steering response (Fig.~\ref{shooter_results}F).

Note that this is not really a functioning FPS bot, as that would
require skills such as finding health packs and navigating the
environment. Future research will investigate whether the DFL approach
