...
\begin{figure}[h!]
  ...
  \caption{Results of the linefollowing task. A) Shows the robot at
    ...
    G) the weights of the output layer and 
    H) the euclidean distance of the weights for each layer from their initial starting point.
    \label{line_results}}
\end{figure}
...
at the end of the run afer 13400 time steps. While the agent in A) clearly
...
and then only small components are left. This error signal has been smoothed out
to see if the average error becomes zero. E) is the weight matrix at the end
of learning of the 1st layer which correlates the error with the two rows of
predictive ground sensors $v_j$.
