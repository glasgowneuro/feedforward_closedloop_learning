...
\begin{abstract}
  ...
  backpropagation (commonly known as ``backprop''), where an error is propagated from the output layer
  back to its input layer .  However,
  ...
  the feedback from the environment propagates it back to the
...
When an agent acts in its environment, its actions will in turn change
its sensor inputs which then in turn cause new actions -- in short,
the agent acts in a closed-loop system. In the simplest case this is a purely
...
propagation mechanism. first proposed theoretically by
Hebb \cite{Hebb49}, and then confirmed many times: for long term
potentiation (LTP) to happen, both pre- and post-synaptic neurons need
...
in the same way as backprop. Eventually the agent will make an
...
\begin{figure}[!ht]
  ...
  \caption{A closed loop system with a setpoint $SP$, transfer
    function $H_0$ and the environment $P_0$ \& $P_1$ which needs to
    work against unpredictable disturbances $D$. The error signal
    tunes a neuronal network, which has inputs $V_i$ that predict the
    disturbances which is achieved by delaying the disturbance $D$ by
    $T$ time steps ($z^{-T}$). The transfer functions here are
    indicated with capital letters and are in the sampled z-domain.
    \label{closed_loop}}
\end{figure}
...
entire closed loop system, with the feedback learning algorithm as a black
...
action is generated with the transfer function $H_0$, which transforms the
...
encountered. In biology the term ``reflex'' means that the behaviour is stereotypical but guarantees that the closed loop
...
possible. The reason we have chosen a feedback loop because it
...
feedback loop via $H_0$ and $P_0$. These additional inputs originate
from the undelayed disturbance $D$ via the environmental transfer
function $P_1$ and appear as additional inputs $V_i$ -- we have
shown one pathway and possible additional inputs as a dashed line in
Fig~.\ref{closed_loop}). For example a video camera or eyes can
...
\begin{figure}[!ht]
  ...
  \caption{A) Network overview. With the exception of the input layer, every
    ...
    to perform correlation ($\times$) based learning and change the weight
...
have drawn one of the learning units in Fig~\ref{netw_together}B, where we see the two processing
...
via the \textsl{same} weights, and finally the weights are
adjusted. Thus, both the error signal and the activity are propagated
in a forward fashion. Learning itself can be interpreted as
\textsl{heterosynaptic} for the activation $v_j$ and \textsl{Hebbian}
for the error $e_j$. This can be seen in Fig~\ref{netw_together}B when
comparing the upper signal flow of the activation from $v_j \to v_k$
which correlates the (weighted) \textsl{input}-signals $v_j$ and $e_j$
with each other to change the weight $w_{jk}$. This is called
heterosynaptic learning. On the other hand the weight change from the
perspective of the error signal is \textsl{Hebbian} because it correlates a
\textsl{pre-synaptic} signal ($v_j$) with a \textsl{post-synaptic}
signal ($e_k$). 
...
\cite{Porr2006ICO}. Because of the exploding complexity of analysing closed loop
...
learning but in closed loop and forward fashion. Unlike deep learning, the algorithm uses correlations only, and does not calculate derivatives. For a stability
...
environmental transfer function is rarely known; this leads us to the application scenarios. 
...
\begin{figure*}[!ht]
  ...
  \caption{Results of the line following task. A) shows the robot at
    ...
    their random initialisation. Parameters: 30 inputs from two
...
decision-making units, with 3 of them determining the change of speed of
...
As a performance criterion we use the absolute value of the error from
Eq.~\ref{line_error}:
\begin{equation}
  \mathrm{error}_\mathrm{abs} =  |\mathrm{error}| \label{line_abserr}
\end{equation}
we define success as having $\mathrm{error}$ below a threshold of $0.001$ for 1000 time steps
-- realistically, driving will never be 100\% perfect, and we
...
of the pixels closest to that colour. We make the gain of the reflex deliberately low so that it is slow to respond; it is also inherently noisy, as other events in the game are also rendered
...
bot would require abilities such as seeking rewards
...
concentration causes long term potentiation (LTP)
\cite{Malenka99,Bennett2000} and a low one causes long term depression
(LTD) \cite{Mulkey1992}. This requires a strong presynaptic drive to
achieve a strong postsynaptic activity, and with that Calcium influx
...
\cite{Canolty2010}. FCL can therefore provide a mechanism that allows
...
Looking at a system-wide perspective FCL, offers an answer to how errors
can propagated further beyond its immediate targets, which has been a mystery
in the neuroscience of decision making. In particular the striatum is
seen as an actor receiving an error signal via dopamine, the well
known reward prediction error \cite{Schultz97}, which is understood as
being closely related to the error of temporal difference (TD)
learning \cite{gurney98:_basal_gangl_action_selec_devic}. It is
curious that the striatum receives the error signal but both
downstream neurons in the basal ganglia and upstream in the cortex
receive much less dopaminergic modulation
\cite{Beckstead1979}. However, plasticity is not just limited to the
striatum, but happens in all brain areas, which requires a mechanism
that is able to propagate this error signal further into the cortex
\cite{Groenewegen1993} where high level decision-making takes place.
In the language of TD learning \cite{Sutton87} this means that we have
a distributed actor where the first layer is modulated by the reward
prediction error, and then the following stages, which that project back to
the cortex, instead use cross frequency coupling as described above
\cite{Lipski2017}.
...
Where we have turned error propagation upside down to use it in a
