...
calculated between the input signals and the error signal. This is similar
to Hebbian learning but here the presynaptic term is the activity
and postsynaptic one is an error signal.
...
weighted by $w_{jk}$ and then summed in the next layer. The same
happens for the error signal from the previous layer $e_j$ which is
also weighted by $w_{jk}$ and then summed in the next layer. Remember
that for the 1st hidden layer, this error signal is just the injected
error signal from the feedback loop.
