...
whose speed is controlled by the feedback control and DFL. The robot
has the task of following the line depicted in
...
where the line follower is using just the ground sensors of the robot
to create the error signal ($g_l$, $g_r$) and the predictive signals
($p_m$) for DFL. Each of the 30 predictive signals $p_m$ from the two
rows of ground sensors is split into 10 2nd-order lowpass filters with
impulse responses lasting from $2$ timesteps to $30$ timesteps, and
then all feed into the DFL network as predictive inputs $v_j$, giving
300 inputs in total. There were 2 hidden layers with 9 and 6 neurons;
6 output neurons. All layers were fully connected. The learning rate
was 0.0001. The four ground sensors in
Fig.~\ref{linefollower_robot_playground}A left/right on either side of
the robot create an error signal:
\begin{equation}
...
threshold KD of 0.15 - Fig.~\ref{shooter_results}B shows this as a
...
aiming response (Fig.~\ref{shooter_results}F).
