...
functions and unroll the partial derivative $\frac{\partial
  V_j}{\partial V_k}$, whose derviative depends again on the entire
...
\begin{figure*}[!ht]
	...
	\caption{A) KD at end of trial. B) Time to success vs learning
          ...
          normalised to zero-mean, unit variance. Learning rate in C, D
          and E was 0.0001.
...
nature of the error signal (Fig.~\ref{shooter_results}E) but stablises
during learning because the long term average error converges to
zero. The different scales are likely due to the inputs having more
dimensions.
...
and generate an aiming response (Fig.~\ref{shooter_results}F).
...
been demonstrated in an abstact network with one hidden layer by
...
decision making, the implied brain connectively is not faithful to
...
they embedd it into Q-learning which is itself closed loop. However,
